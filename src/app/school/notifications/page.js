...
export default function SchoolNotificationsPage() {
  ...
  useEffect(() => { load(); }, []);
  const markAll = async () => { try { await api.patch("/api/notifications", {}); toast.success("All read"); load(); } catch (e) { toast.error(e.message); } };
  return (
    <div>
      <PageHeader title="Notifications" subtitle="Approval and result alerts"
        action={<Button variant="outline" onClick={markAll}><CheckCheck className="h-4 w-4" /> Mark all read</Button>} />
      {loading ? <Spinner /> : items.length === 0 ? <EmptyState title="No notifications yet" /> : (
